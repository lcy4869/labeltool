...
class UserView(sqla.ModelView):
    def approve(self,users):
        for user in users:
            print 1
            # print user.id'
        return '<a href="/admin/">Click me to get to Admin!</a>'
    @action('alert', 'Approve', 'Are you sure you want to approve selected users?')
    def action_approve(self, ids):
        try:
            ...
            # return '<script>disp_prompt();function disp_prompt(){ var name=prompt("input a username","lcy4869@qq.com") \
            # if (name!=null && name!="")\
            # {document.write("asd" + name + " asd")\
            # }}</script>'
            return redirect('/batch')

            # self.approve(query.all())
            # for user in query.all():
            #     print user.id
